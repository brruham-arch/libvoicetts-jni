-- voicetts.lua v2.8 by brruham
-- Google TTS JNI -> WAV -> BASS stream (speaker) + mic inject (SampVoice)
...
local function saveConfig() inicfg.save(ini, CFG_FILE) end
...
local chkMicInject = new.bool(ini.settings.mic_inject ~= false)
...
local function speak(text)
    ...
    if tts.is_ready() == 0 then
        sampAddChatMessage("[TTS] Belum ready...", 0xFFAA00); return
    end
    text = text:match("^%s*(.-)%s*$")
...
imgui.OnInitialize(function()
    ...
    s.WindowRounding = 6.0; s.FrameRounding = 4.0
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.Begin("VoiceTTS v2.8", showSettings,
        ...
        if imgui.Button(ttsOn and "TTS OFF" or "TTS ON", imgui.ImVec2(280,30)) then
            chkEnabled[0] = not ttsOn
            ini.settings.enabled = chkEnabled[0]; saveConfig()
            if tts then if chkEnabled[0] then tts.enable() else tts.disable() end end
        ...
        -- Play Local toggle
        if imgui.Checkbox("Speaker Game (TikTok)", chkPlayLocal) then
            ini.settings.play_local=chkPlayLocal[0]; saveConfig()
            if tts then tts.set_play_local(chkPlayLocal[0] and 1 or 0) end
            sampAddChatMessage("[TTS] Speaker: "..(chkPlayLocal[0] and "ON" or "OFF"),
                chkPlayLocal[0] and 0x00FF88 or 0xAAAAAA)
        end

        -- Mic Inject toggle
        if imgui.Checkbox("Mic Inject (SampVoice)", chkMicInject) then
            ini.settings.mic_inject=chkMicInject[0]; saveConfig()
            if tts then tts.set_mic_inject(chkMicInject[0] and 1 or 0) end
            sampAddChatMessage("[TTS] Mic Inject: "..(chkMicInject[0] and "ON" or "OFF"),
                chkMicInject[0] and 0x00FF88 or 0xAAAAAA)
        end

        imgui.Spacing(); imgui.Separator()
        if imgui.Button(showInput[0] and "Tutup Input" or "Buka Input", imgui.ImVec2(280,28)) then
            if chkEnabled[0] then showInput[0]=not showInput[0]
...
imgui.OnFrame(
    ...
    function()
        ...
        imgui.SetNextWindowPos(imgui.ImVec2(sizeX*0.5-170,sizeY*0.25), imgui.Cond.FirstUseEver)
...
function main()
    ...
    wait(2000)
    sampAddChatMessage("[VoiceTTS] v2.8 loading...", 0xFFFF00)
    ...
    for i = 1, 10 do
        ...
        if f then
            ...
            if addr and addr~=0 then
                ...
                if ok and api then tts=api; os.remove(ADDR_FILE); break end
            end
        end
        wait(1000)
    end
    ...
    tts.set_mic_inject(chkMicInject[0] and 1 or 0)
    tts.set_play_local(chkPlayLocal[0] and 1 or 0)
    if chkEnabled[0] then tts.enable() else tts.disable() end

    for i = 1,10 do if tts.is_ready()==1 then break end; wait(500) end
    sampAddChatMessage("[TTS] v2.8 OK | Speaker+MicInject | /ttsui", 0x00FF88)

    sampRegisterChatCommand("ttsui", function() showSettings[0]=not showSettings[0] end)

    sampRegisterChatCommand("tts", function(arg)
        if arg=="on" then
            chkEnabled[0]=true; tts.enable(); ini.settings.enabled=true; saveConfig()
            sampAddChatMessage("[TTS] ON", 0x00FF88)
        elseif arg=="off" then
            chkEnabled[0]=false; tts.disable(); showInput[0]=false
            ini.settings.enabled=false; saveConfig()
            sampAddChatMessage("[TTS] OFF", 0xFF8800)
        elseif arg and #arg>0 then speak(arg)
        else sampAddChatMessage("[TTS] /tts <text>|on|off | /ttsui", 0xFFFF00) end
    end)
    ...
    sampRegisterChatCommand("ttspitch", function(arg)
        local v=tonumber(arg); if v and tts then
            sliderPitch[0]=v; ini.settings.pitch=v; saveConfig()
            sampAddChatMessage("[TTS] Pitch="..string.format("%.1f",v), 0x00FFFF) end
    end)

    sampRegisterChatCommand("ttsrate", function(arg)
        local v=tonumber(arg); if v and tts then
            sliderRate[0]=v; ini.settings.rate=v; saveConfig()
            sampAddChatMessage("[TTS] Rate="..string.format("%.1f",v), 0x00FFFF) end
    end)

    sampRegisterChatCommand("ttsvol", function(arg)
        local v=tonumber(arg); if v and tts then
            sliderVol[0]=math.floor(v); tts.set_volume(math.floor(v))
    ...
    sampRegisterChatCommand("ttslang", function(arg)
        if arg and #arg>0 and tts then
            tts.set_lang(arg); ini.settings.lang=arg; saveConfig()
            for i,v in ipairs(LANGS) do if v==arg then langIdx[0]=i-1; break end end
            sampAddChatMessage("[TTS] Lang="..arg, 0x00FFFF)
        else sampAddChatMessage("[TTS] /ttslang id|en|ms|en-US|...", 0xFFFF00) end
    end)

    sampRegisterChatCommand("ttsmic", function()
        if not tts then return end
        chkMicInject[0] = not chkMicInject[0]
        tts.set_mic_inject(chkMicInject[0] and 1 or 0)
        ini.settings.mic_inject=chkMicInject[0]; saveConfig()
        sampAddChatMessage("[TTS] Mic Inject: "..(chkMicInject[0] and "ON" or "OFF"),
            chkMicInject[0] and 0x00FF88 or 0xAAAAAA)
